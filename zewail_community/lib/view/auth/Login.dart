...
import 'component/ResetPasswordDialog.dart';
import 'component/customcomponent.dart';

class Login extends StatefulWidget {
  String? phone;

  Login({super.key, this.phone});
  @override
...
//  final firbasemessag = FirebaseMessaging.instance;
  ...
  void initState() {
    widget.phone != null
        ? studntNumber.text = widget.phone!
        : studntNumber.text = '';
        ...
        child: BlocConsumer<LoginCubit, LoginStates>(
          listener: (context, state) {
            if (state is LoginLoadinglState) {
              isloading = true;
            } else if (state is LoginSucceslState) {
              if (state.stdata != null) {
                prefs!.setString("token", state.stdata.accessToken!);
              }
              print('token api =${prefs!.getString("token")}');
              //  fetchFCMToken();

              prefs!.setInt("stid", state.stdata.student!.stId!);
              prefs!.setString("stname", state.stdata.student!.stName!);
              prefs!.setBool("ist", state.stdata.isTeacher!);
              prefs!.setString("pass", state.stdata.student!.haspass!);
              if (state.stdata.student!.cardCode != null) {
                prefs!.setInt("code", state.stdata.student!.cardCode!);
                print('${state.stdata.student!.cardCode!}');
              }
              if (state.stdata.student!.profile != null) {
                prefs!.setString("profile", state.stdata.student!.profile!);
              }
                      ...
                      child: Padding(
                        ...
                        child: ListView(
                          ...
                          children: [
                            Center(
                              child: Text(
                                "(للطلاب فقط) سجل الدخول الأن بحسابك",
                                style: TextStyle(
                                    fontSize: 20,
